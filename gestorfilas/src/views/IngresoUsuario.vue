...
<v-container fluid ma-0 pa-0 fill-height class="cyan lighten-5">
  ...
  <v-card ref="form" class="mx-auto pa-md-4" 
    max-width="500">
    <div>
    <h2>
        Ingresa tus datos para unirte a la fila virtual
    </h2>
    </div>
    <v-text-field
      v-model="name"
    ...
    <v-btn class="mr-4 white--text" color=#00C2CB @click="submit">Unirse a la fila</v-btn>
    <v-btn @click="clear"  class="white--text" color=#00C2CB>Limpiar campos</v-btn>
  </v-card>
    </v-layout>
</v-container>
</template>
  ...
  export default {
    ...
    computed: {
      nameErrors () {
        ...
        !this.$v.name.maxLength && errors.push('El nombre puede tener máximo 20 caracteres.')
      ...
      emailErrors () {
        ...
        !this.$v.email.required && errors.push('Debe ingresar un correo electrónico.')
        return errors
      },
    },
